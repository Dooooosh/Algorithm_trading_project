...
trades_filename = os.path.join(output_dir, 'binance_trades_big.csv')
    ...
    async with connect(uri) as websocket:
        while True:
            try:
                ...
                trade_time = datetime.fromtimestamp(data['T']/ 1000, pytz.timezone('Asia/Bangkok'))
                ...
                print(f"Error in binance_trade_stream: {e}")
...
async def main():
    trade_stream_tasks = [binance_trade_stream(f"{websocket_url_base}{symbol}@aggTrade", symbol, trades_filename, trade_aggregator) for symbol in symbols]
